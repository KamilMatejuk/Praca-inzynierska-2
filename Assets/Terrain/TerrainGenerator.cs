...
public class TerrainGenerator : MonoBehaviour {
    ...
    [SerializeField] public TerrainGenData terrainGenData;
            ...
            if (controlPoints != null && controlPoints.terrainGenData != null) {
                ...
                for (int i = 0; i < controlPoints.NumberOfSegments; i++) {
                    ...
                    for (float t = 0; t < 1; t += 0.01f) {
                        ...
                        op.position += new Vector3(Variables.TERRAIN_SIZE / -2, 0, Variables.TERRAIN_SIZE / -2);
    ...
    public void GenerateTerrainShape() {
        UnityEngine.Random.InitState(seed);
        GetComponent<Terrain>().terrainData.heightmapResolution = Variables.TERRAIN_SIZE + 1;
        GetComponent<Terrain>().terrainData.size = new Vector3(Variables.TERRAIN_SIZE, Variables.TERRAIN_HEIGHT, Variables.TERRAIN_SIZE);
        float[,] heights = new float[Variables.TERRAIN_SIZE, Variables.TERRAIN_SIZE];
        for (int xi = 0; xi < Variables.TERRAIN_SIZE; xi++) {
            for (int zi = 0; zi < Variables.TERRAIN_SIZE; zi++) {
    ...
    public static TerrainLayer GetTerrainLayerColor(Color color) {
        ...
        terrainLayer.metallic = 0f;
        terrainLayer.smoothness = 0f;
        terrainLayer.name = "color_" + Parser.ColorToString(color);
        return terrainLayer;
    }
    ...
    void PutStartFinish() {
        ...
        Objects.RemoveObjectsByTagInParent("meta", gameObject);
        startFinish = Objects.PutObject("StartFinish", "meta", "Meta", op);
    ...
    void PutCars(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            op.position = carPos;
            GameObject car = Objects.PutObject("SportCar", "car", "Car " + i, op);
    ...
    void PutCheckpoints() {
        ...
        GameObject checkpointsGroup = Objects.PutParentObject("checkpoint", "Checkpoints");
        checkpointsGroup.transform.parent = transform;
        for (int i = 0; i < ops.Count; i++) {
            OrientedPoint op = ops[i];
            op.rotation *= Quaternion.Euler(0, 90, 0);
            GameObject chpt = Objects.PutObject("Checkpoint", "checkpoint", "Checkpoint " + i, op);
